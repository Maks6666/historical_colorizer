...
class ColorizerV7(nn.Module):
    ...
    # def embadding(self, rgb_image):
    #     out = self.pretrained_model(rgb_image)
    #     out = F.leaky_relu(self.linear(out))

        # return out

    def predict(self, l_channel, rgb_image):
        self.eval()

        with torch.no_grad():
            # embaddings = self.embadding(rgb_image)
            ab_channels = self.forward(l_channel, rgb_image)
...
def emb_model_1():
    model = ColorizerV7()
    model.load_state_dict(torch.load("weights/history_in_color_v13.pt", map_location=device))
    return model
...
embadding_tensor = torch.rand(1, 3, 256, 256)
embadding = emb_model.predict(embadding_tensor)

res = model.predict(tensor, embadding)
print(res.shape)
